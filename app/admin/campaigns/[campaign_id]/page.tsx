...
import { getAdminCampaignById } from "@/services/admin/campaigns.service";
...
export default async function CampaignDetailPage({ params }: CampaignDetailPageProps) {
  ...
  const campaign = await getAdminCampaignById(campaign_id);
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
        <DataPanel
          ...
          description={
            <p className="max-w-2xl text-sm text-zinc-400">
              Campaign detail remains read-first and focused on targeting, rules, and performance posture.
            </p>
          }
        >
          <dl className="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
            <div>
              ...
              <dd className="mt-2 break-all font-mono text-zinc-300">{campaign.campaign_id}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">Status</dt>
              <dd className="mt-2 break-words capitalize text-zinc-300">{campaign.status}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">Type</dt>
              <dd className="mt-2 break-words capitalize text-zinc-300">{campaign.type}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">Reward Type</dt>
              <dd className="mt-2 break-words text-zinc-300">{campaign.reward_type}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">Start</dt>
              <dd className="mt-2 text-zinc-300">{new Date(campaign.start_at).toLocaleString()}</dd>
            </div>
            <div>
              <dt className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500">End</dt>
              <dd className="mt-2 text-zinc-300">{new Date(campaign.end_at).toLocaleString()}</dd>
            </div>
          </dl>
        </DataPanel>
        ...
        <div className="space-y-6">
          <DataPanel title={<h2 className="text-xl font-semibold text-white">Performance</h2>}>
            <p className="text-3xl font-semibold text-white">{campaign.participants}</p>
            <p className="mt-2 text-sm text-zinc-400">Current participant count</p>
            <p className="mt-4 break-words text-sm text-zinc-300">{campaign.performance_summary}</p>
          </DataPanel>

          <DataPanel title={<h2 className="text-xl font-semibold text-white">Targeting</h2>}>
            <p className="break-words text-sm text-zinc-300">{campaign.targeting_summary}</p>
          </DataPanel>
